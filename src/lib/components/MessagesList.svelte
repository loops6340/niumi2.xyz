<script lang="ts">
	...
	let { messages, updateMessages, updateToPreviousMessages }: Props = $props();
</script>
...
<div 
  ...
  class="flex min-h-0 flex-1 flex-col-reverse items-start gap-2 overflow-auto pt-2 pb-2">
	<div 
    class="absolute bottom-18 flex flex-col right-3 md:right-90 2xl:right-120 gap-2"
  >
		...
		{#if message.content || message.attachments[0]?.url}
			<div class="flex max-w-full items-start gap-2 pl-2">
				{#if message.author.id !== '1073726760350392340'}
					...
					/>
				{:else}
					<div class="h-12.5 w-12.5 min-w-12.5 border border-black bg-fuchsia-200"></div>
				{/if}
